...
from cogs import stablecog
from cogs import upscalecog
        ...
        if invalid_input:
            ...
            if settings.global_var.prompt_ban_list or settings.global_var.prompt_ignore_list or settings.global_var.negative_prompt_prefix:
                ...
                if settings.global_var.prompt_ban_list and mod_results[0] == "Stop":
                    await interaction.response.send_message(f"I'm not allowed to draw the word {mod_results[1]}!",
                                                            ephemeral=True)
                    return
                if settings.global_var.prompt_ignore_list or settings.global_var.negative_prompt_prefix and mod_results[
                    0] == "Mod":
            ...
            if str(pen[18]) != 'None':
                if str(pen[18]) != str(
                        self.input_tuple[18]) and new_net_multi != net_multi or new_net_multi != net_multi:
            ...
            if queuehandler.GlobalQueue.dream_thread.is_alive():
                queuehandler.GlobalQueue.queue.append(
                    queuehandler.DrawObject(stablecog.StableCog(self), *prompt_tuple, DrawView(prompt_tuple)))
            else:
                await queuehandler.process_dream(draw_dream,
                                                 queuehandler.DrawObject(stablecog.StableCog(self), *prompt_tuple,
                                                                         DrawView(prompt_tuple)))
            ...
            if buttons_free:
                ...
                if queuehandler.GlobalQueue.dream_thread.is_alive():
                    if user_queue_limit == "Stop":
                        ...
                        queuehandler.GlobalQueue.queue.append(
                            queuehandler.DrawObject(stablecog.StableCog(self), *seed_tuple, DrawView(seed_tuple)))
                else:
                    await queuehandler.process_dream(draw_dream,
                                                     queuehandler.DrawObject(stablecog.StableCog(self), *seed_tuple,
                                                                             DrawView(seed_tuple)))
        ...
        try:
            ...
            if buttons_free:
                ...
                if batch[0] != 1 or batch[1] != 1:
                    await interaction.response.send_message("Use the drop down menu to upscale batch images!",
                                                            ephemeral=True)  # tell user to use dropdown for upscaling
                else:
                    ...
                    upscale_tuple = (ctx, '2.0', init_image, upscaler_1, "None", '0.5', '0.0', '0.0',
                                     False)  # Create defaults for upscale. If desired we can add options to the per channel upscale settings for this.
                    ...
                    if queuehandler.GlobalQueue.dream_thread.is_alive():
                        if user_queue_limit == "Stop":
                            ...
                            queuehandler.GlobalQueue.queue.append(
                                queuehandler.UpscaleObject(upscalecog.UpscaleCog(self), *upscale_tuple,
                                                           DeleteView(upscale_tuple)))
                    else:
                        await queuehandler.process_dream(draw_dream,
                                                         queuehandler.UpscaleObject(upscalecog.UpscaleCog(self),
                                                                                    *upscale_tuple,
                                                                                    DeleteView(upscale_tuple)))
...
class DownloadMenu(discord.ui.Select):
    def __init__(self, epoch_time, seed, batch_count, input_tuple):
        ...
        options = [discord.SelectOption(label=option[1], value=option[0], description=option[0]) for option in
                   input_options]
        super().__init__(custom_id="download_menu", placeholder='Choose images to download...', min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            if buttons_free:
                ...
                if files:
                    await interaction.response.send_message(
                        f'<@{interaction.user.id}>, please wait I am fetching your requested images', view=None)
                    blocks = [files[i:i + 10] for i in range(0, len(files), 10)]
                    for block in blocks:
                        await interaction.followup.send(
                            f'<@{interaction.user.id}>, Here are the batch files you requested', files=block,
                            view=DeleteView(self.input_tuple))
            else:
...
class UpscaleMenu(discord.ui.Select):
    def __init__(self, epoch_time, seed, batch_count, input_tuple):
        ...
        options = [discord.SelectOption(label=option[1], value=option[0], description=option[0]) for option in
                   input_options]
        super().__init__(custom_id="upscale_menu", placeholder='Choose images to upscale...', min_values=1,
        ...
        try:
            ...
            if buttons_free:
                ...
                if queuehandler.GlobalQueue.dream_thread.is_alive():
                    ...
                    await queuehandler.process_dream(draw_dream, queuehandler.UpscaleObject(upscalecog.UpscaleCog(self),
                                                                                            *upscale_tuple,
                                                                                            DeleteView(upscale_tuple)))
