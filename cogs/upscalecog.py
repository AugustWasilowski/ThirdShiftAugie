...
class UpscaleCog(commands.Cog):
    ...
    @discord.slash_command(name='upscale', description='Upscale an image', guild_only=True)
    @option(
        'init_image',
        discord.Attachment,
                ...
                with io.BytesIO() as buffer:
                    ...
                    queuehandler.process_post(
                        self, queuehandler.PostObject(
                            self, queue_object.ctx, content=f'<@{queue_object.ctx.author.id}>, {message}', file=file,
                            embed='', view=queue_object.view))
