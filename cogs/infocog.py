...
class InfoView(View):
        ...
        for i in range(0, length, batch * 2):
            ...
            if length > batch * 2:
                embed_page.set_footer(
                    text=f'Page {self.page + 1} of {math.ceil(length / (batch * 2))} - {length} total')
            self.contents.append(embed_page)
            self.page += 1
        self.page = 0
    ...
    async def button_tips(self, _button, interaction):
        ...
        embed_tips1 = discord.Embed(title="Documentation",
                                    description="Welcome to the documentation! Basic usage and prompting tips is explained here.",
                                    colour=settings.global_var.embed_color)
        ...
        embed_tips3 = discord.Embed(title="Basic Prompting Tips",
                                    description="This is some of the syntax that can be used with your prompts.",
                                    colour=settings.global_var.embed_color)
        ...
        embed_tips4 = discord.Embed(title="Buttons",
                                    description="Generated images contain some neat, convenient buttons!",
                                    colour=settings.global_var.embed_color)
...
class InfoCog(commands.Cog):
    ...
    @discord.slash_command(name="info", description="Lots of useful information or download batch images!")
    @option(
        'batch_id',
        str,
    ...
    async def info(self, ctx, batch_id: Optional[str] = None, image_id: Optional[str] = None):
        if not batch_id and not image_id:
            ...
            if files:
                ...
                for block in blocks:
                    await ctx.respond(f'<@{ctx.author.id}>, Here are the batch files you requested', files=block,
                                      view=view)
            else:
